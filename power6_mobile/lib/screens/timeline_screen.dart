...
class _TimelineScreenState extends State<TimelineScreen> {
  ...
  Future<void> _loadTaskHistory() async {
    ...
    try {
      ...
      if (res.isSuccess && res.data != null) {
        ...
        final List<Task> tasks = (listDyn as List<dynamic>? ??
                const <dynamic>[])
  ...
  Future<void> _loadAnalytics() async {
    ...
    if (!mounted) return;
    if (res.isSuccess) {
      _analytics.track('pro_insights_viewed', token: token);
    }
    setState(
        () => _analyticsFuture = Future.value(res.isSuccess ? res.data : null));
  }
  ...
  Future<void> _exportCsv() async {
    ...
    _analytics.track('csv_export_started', token: token);
    final res = await _insights.exportCsv(token: token);
    if (!mounted) return;
    if (!res.isSuccess) {
      messenger
          .showSnackBar(SnackBar(content: Text(res.error ?? 'Export failed.')));
      return;
    }
    final csv = res.data?['raw']?.toString() ?? '';
    if (csv.isEmpty) {
      messenger
          .showSnackBar(const SnackBar(content: Text('No CSV data returned.')));
      return;
    }
              ...
              children: <Widget>[
                ...
                Wrap(
                  ...
                  children: <Widget>[
                    ...
                    _InsightChip(
                        label: 'Best day', value: '$bestDay ($bestCount)'),
                  ],
                ),
              ],
            );
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 12),
                ElevatedButton(
                    onPressed: onRetry, child: const Text('Refresh')),
              ],
            ),
          ),
        ),
