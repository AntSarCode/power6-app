...
class _BadgeScreenState extends State<BadgeScreen> {
  // Start with an empty list so the UI never throws when no data exists
  Future<List<userbadge.Badge>> _badges = Future.value(<userbadge.Badge>[]);
  String? _errorMessage; // Hold a friendly error for display without crashing
  ...
  Future<void> _loadBadges() async {
    setState(() {
      _errorMessage = null; // reset error on each load
    });

    final token = context.read<AppState>().accessToken ?? '';

    // If not logged in yet, render a calm empty state instead of an error
    if (token.isEmpty) {
    ...
    try {
      ...
      if (response.isSuccess) {
        ...
        // Treat typical network/config cases as a soft empty with a friendly notice
        setState(() {
      ...
      // ClientExceptions, timeouts, or bad base URLs land here
      setState(() {
              ...
              child: IntrinsicHeight(
                child: FutureBuilder<List<userbadge.Badge>>(
                  ...
                  builder: (context, snapshot) {
                    ...
                    if (items.isEmpty) {
                      // Distinguish between truly-no-badges and temporarily unavailable
